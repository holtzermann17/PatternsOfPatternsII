...
synthesis of three techniques from training, futures studies, and
design in the form of a design pattern called PLACARD
...
The Project Action Review is an adaptation of the After Action Review
to horizontally-managed group work; the AAR has seen widespread use in
military, business, and healthcare settings, and has proved to be an
effective training approach in task environments that are
characterized by a combination of high complexity and ambiguity
\cite{Keiser2021,Keiser2022}.  Causal Layered Analysis has been used
for over thirty years by members of the futuring community to scaffold
change-making initiatives \cite{cla3}.  Design Pattern Languages are
presumed to be well known to readers of this paper, and we offered a
novel analysis of the pattern languages discourse in the prequel.
There, we made the further assertion that these sensory, cognitive,
and motor methods can be hooked together, theorizing design patterns
as little pieces of moveable social intelligence.  We called the
specific method that combines PAR, CLA, and DPL the “PLACARD” pattern.
We built on case study analyses to outline potential futures for the
development of pattern methods.
%% \begin{itemize}
...
peeragogy were previously presented in design pattern form
...
microcosm.
The overall aim of this line of work is to study and support social
intelligence \cite{benkler2015peer}, with a particular emphasis on
learning aspects.  Design patterns are a useful medium.
...
“living language” from the start \cite[p.~xvii]{alexander1977a}, a
well-articulated evolutionary perspective on pattern language has the
potential to be transformative at the practice level.
\citet[p.~xxiii]{lowy2019righting} notes that no other industry, apart
from software development, opts for a clean slate on a regular basis,
because doing so does not make economic sense.

With this framing in mind, we explore how design patterns, design
pattern languages, and associated methods evolve and adapt over time.
...
\end{center}
\vspace{-1cm}
\caption{Selected updated pattern cards, representing 14 patterns in manipulative form (with two “blanks”)\label{revised-cards}}
\end{figure}
\FloatBarrier
...
\noindent Although the language is different (cf.~{\sc Adapt Layers As
  Needed}), a comparison with CLA is possible, as both frameworks
reach from direct and mundane observables, on the one hand, to the
underlying or overarching reasons and motivations for observed
behaviour, on the other.

By applying PLACARD to itself (as it were), {\sc Going Meta} allows us
