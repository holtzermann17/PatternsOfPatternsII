...
“living language” \cite[p.~xvii]{alexander1977a} from the start,
readers may nevertheless find well-articulated evolutionary
perspective on pattern language development both practical and
illuminating.  In particular, \citet[p.~xxiii]{lowy2019righting} notes
that “no other industry [apart from software development!] opts for a
clean slate on a regular basis simply because doing so does not make
economic sense.”
...
the other workshops, via a new visual template which bears some
similarity to the “wholeness egg” of \citet{wholeness-egg-II}.
...
selection of the patterns that we used are included in this paper.
...
also include the itinerary for each of the workshops to help bring the
...
neuroscientific language that underpins PLACARD.  It might integrate
...
needs to be revised to match a previously-unexpected reality;
...
\noindent \citet{benoit2019structure} supply this paraphrase of
...
average.”  Similar thinking might characterise the evolution of
pattern languages, both in urban contexts and other settings.
Developments along these lines could help correct the structural
limitations of \emph{A Pattern Language} described by
\cite{Dawes2018}.
...
that AIF and DPL can support each other, with AIF being given a
practical and intuitive implementation using design pattern concepts,
and DPL receiving a needed theoretical through integration with Active
Inference.  Both are loosely hierarchical in nature: in the large,
Alexander’s theory zooms in from patterns for towns, to buildings, to
construction; whereas AIF zooms out, from “I model the world” to “we
model the world” to “we model ourselves modelling the world”
\cite{Kirchhoff2018}.
...
The primary contributions made in this paper can be summarised thusly:

\begin{itemize}
\item Table \ref{summary} pulls togetherthe patterns we have described
  from across the separate case studies, and organizes them in groups,
  elaborating our previous use of “PAR”, “CLA”, and “DPL” methods
  (summarized in Section \ref{sec:org7c32ecc}) with \emph{a rounded
  outline of the purposes that sensory, cognitive, and motor methods
  serve when working with design patterns.}
...
\item Figure \ref{revised-cards} re-presents selected patterns as
  described in this paper, in \emph{a practical format in which the
  material presented here can be used to co-design future
  workshops}.\footnote{``Open Future Design Workshop: Together with
  workshop participants, we will co-develop a Design Pattern Language
  for envisioning, exploring, and enacting the future.”  Wikimania
  2023, \url{https://pretalx.com/wm2023/talk/WVLJRV/}} Whilst the
  collection of patterns has been refined through practice and
...
\end{center}
\vspace{-1cm}
\caption{Selected pattern cards, representing the patterns in manipulative form\label{revised-cards}}
\end{figure}
\FloatBarrier
...
way that makes it more robust.  This relative success notwithstanding,
it is worth recalling that the scope of “Patterns of Patterns” was
larger: our expressed aim was to support distributed collaboration
across contexts.

Although the work presented here was wide-ranging and participatory,
we have not yet implemented a fully distributed peer-to-peer
collaboration platform.  However, we have created something like a
client-server simulation of one.  For most workshop participants, the
workshop has happened once: whereas for the facilitators, it has
repeated with variation over time, allowing us to learn.  Our primary
gambit has been to seek insights from the workshop pilots which can
