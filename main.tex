...
the need and plan for evolution at the process level.  Presently, with
Table \ref{active-inference-factors} we draw attention to the way in
which the three main participant roles operationalize key aspects of
the Active Inference Framework (see \citet{SMITH2022102632} for a
primer on the key concepts in AIF).
...
themselves, so that pattern or pattern language would be able to
...
\noindent \citet{benoit2019structure} paraphrase Christopher
Alexander’s conception of urban development: “‘natural’ cities evolve
by maintaining their amount of surprisal constant on average.”
Similar thinking might characterise the evolution of pattern
languages, both in urban contexts and other settings.  Developments
along these lines could help correct the structural limitations of
\emph{A Pattern Language} (\emph{APL}) described by \citet{Dawes2018}.
...
and DPL receiving a needed theoretical upgrade from Active Inference.
Both are loosely hierarchical: e.g., \emph{APL} zooms in from patterns
...
modelling the world” \cite{Kirchhoff2018}.  Both are, above all,
generative \cite{alexander1999a}.
...
way that makes it fairly robust.  That success notwithstanding, it is
worth recalling that the scope of “Patterns of Patterns” was larger:
our expressed aim was to support distributed collaboration across
contexts.

Although the work presented here was both wide-ranging and
participatory, we have not yet implemented a fully distributed
peer-to-peer collaboration platform.  However, we have created
something like a client-server simulation of one.  For most workshop
participants, the workshop has happened once: whereas for the
facilitators, it has repeated with variation over time, allowing us to
learn.  Our strategy has been to seek insights from the workshop
pilots which can contribute to the design of a platform for
distributed collaboration, building on the analogy between wikis and
pattern languages \cite{cunningham2013a}.
